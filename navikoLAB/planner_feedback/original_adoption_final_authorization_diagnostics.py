...
PHASE = "Phase29-2 Original Adoption Final Authorization Diagnostics"
...
def latest_authorization_record() -> Path | None:
    base = workspace_dir()
    if not base.exists():
        return None

    candidates = sorted(
        base.glob("original_adoption_final_authorization_*.json"),
...
def build_diagnostics(record: dict, source_path: Path | None) -> dict:
    required_checks = {
        "FinalAuthorizationRecordCreated": record.get("FinalAuthorizationRecordCreated") is True,
    ...
    return {
        ...
        "NextPhase": (
            "Phase29-3 Original Adoption Final Authorization Completion Report"
            if diagnostics_passed
            else "Fix Original Adoption Final Authorization Record"
        ),
    }
...
def save_diagnostics(diagnostics: dict) -> Path:
    ...
    out_path = out_dir / f"original_adoption_final_authorization_diagnostics_{timestamp}.json"
    out_path.write_text(
...
def main() -> None:
    source_path = latest_authorization_record()
    ...
    if source_path is None:
        diagnostics = {
            ...
            "NextPhase": "Create Original Adoption Final Authorization Record",
        }
    else:
    ...
    print("=== Original Adoption Final Authorization Diagnostics ===")
