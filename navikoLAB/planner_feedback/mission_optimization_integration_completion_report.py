...
def find_latest_diagnostics():
    files = sorted(
        REPORT_DIR.glob("mission_optimization_integration_diagnostics_*.json"),
...
def main():
    ...
    diagnostics_ok = diagnostics.get("MissionOptimizationIntegrationReady") is True
    risk_count = diagnostics.get("RiskCount", 1)

    output = {
        "status": "completed" if diagnostics_ok and risk_count == 0 else "blocked",
        "phase": "Phase18-6 Mission Optimization Integration Completion Report",
        "DiagnosticsFound": diagnostics_path is not None,
        "DiagnosticsConfirmed": diagnostics_ok,
        "MissionOptimizationIntegrationCompleted": diagnostics_ok and risk_count == 0,
        ...
        "PlannerReadOnly": diagnostics.get("PlannerReadOnly"),
        "CapabilityReadOnly": diagnostics.get("CapabilityReadOnly"),
        "ConnectorReadOnly": diagnostics.get("ConnectorReadOnly"),
        "WorkspaceOnly": True,
        "OriginalWrite": False,
        ...
        "NextPhase": "Phase19 Mission Policy Stabilization",
        "updated_at": datetime.now().isoformat(),
        "DiagnosticsPath": str(diagnostics_path) if diagnostics_path else None,
    }

    out_path = REPORT_DIR / f"mission_optimization_integration_completion_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    out_path.write_text(
        json.dumps(output, ensure_ascii=False, indent=2),
        encoding="utf-8"
    )

    print("=== Mission Optimization Integration Completion Report ===")
