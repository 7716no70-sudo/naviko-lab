...
REQUIRED_FILES = [
    GATE_DIR / "human_approval_gate_completion_report.json",
]

OUTPUT_PATH = APPROVAL_DIR / "explicit_human_approval_record_source.json"
...
def main():
    ...
    gate = collected.get("human_approval_gate_completion_report.json", {})

    result = {
        "status": "completed",
        "phase": "Phase27-1 Explicit Human Approval Record Source Collector",
        "SourceCount": len(collected),
        "MissingSourceCount": len(missing),
        "MissingSources": missing,
        "ReadyForExplicitHumanApproval": gate.get("ReadyForExplicitHumanApproval") is True,
        ...
        "SafeToContinue": (
            len(missing) == 0
            and gate.get("ReadyForExplicitHumanApproval") is True
            and gate.get("RiskCount") == 0
            and gate.get("SafeToContinue") is True
            and gate.get("OriginalWrite") is False
            and gate.get("OriginalWriteBlocked") is True
        ),
    ...
    print("=== Explicit Human Approval Record Source Collector ===")
