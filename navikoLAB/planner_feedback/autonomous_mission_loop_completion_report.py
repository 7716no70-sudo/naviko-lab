...
PHASE = "Phase38-3 Autonomous Mission Loop Completion Report"
...
def load_latest_diagnostics():
    files = sorted(
        REPORT_DIR.glob("autonomous_mission_loop_diagnostics_*.json"),
...
def main():
    ...
    diagnostics_confirmed = bool(
        ...
        and diagnostics.get("MissionLoopReady") is True
        and diagnostics.get("RequiredStepsComplete") is True
    ...
    mission_loop_completed = diagnostics_found and diagnostics_confirmed

    report = {
        "status": "completed" if mission_loop_completed else "blocked",
        "phase": PHASE,
        "DiagnosticsFound": diagnostics_found,
        "DiagnosticsConfirmed": diagnostics_confirmed,
        "AutonomousMissionLoopCompleted": mission_loop_completed,
        "AutonomousMissionLoopReady": mission_loop_completed,
        ...
        "RiskCount": 0 if mission_loop_completed else 1,
        "SafeToContinue": mission_loop_completed,
        "NextPhase": "Phase39 Experience Learning Engine"
        if mission_loop_completed
        else "Review Phase38-2 Diagnostics",
        "source_diagnostics_path": str(diagnostics_path) if diagnostics_path else None,
    }

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_path = REPORT_DIR / f"autonomous_mission_loop_completion_report_{ts}.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    print("=== Autonomous Mission Loop Completion Report ===")
