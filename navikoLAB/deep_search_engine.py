...
")
            ...
            print(f"📋 サブクエリ生成: {len(sub_queries)}個
")
...
'):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_query):
                ...
                try:
                    ...
                    print(f"  ❌ 失敗: {query[:50]}... - {e}")
                    results.append({"query": query, "answer": f"エラー: {str(e)}", "provider": "none", "success": False})
        return results
    ...
    def _sequential_search(self, queries: List[str], providers: List[str]) -> List[Dict[str, Any]]:
        ...
        for i, query in enumerate(queries):
            ...
            results.append(result)
            print(f"  ✅ 完了 ({i+1}/{len(queries)}): {query[:50]}...")
        return results
    
    def _execute_single_query(self, query: str, provider: str) -> Dict[str, Any]:
        """単一クエリを実行"""
        try:
            result = self.connector.generate_code(prompt=query, provider=provider, max_tokens=1000)
            return {
                "query": query,
            ...
            return {"query": query, "answer": f"エラー: {str(e)}", "provider": "none", "success": False}
    
    def _synthesize_results(self, results: List[Dict[str, Any]], original_query: str) -> Dict[str, Any]:
        """複数の結果を統合"""
        valid_results = [r for r in results if r.get("success", False)]
        if not valid_results:
            return {"answer": "情報を取得できませんでした。", "confidence": 0.0, "sources": []}
        
        combined_answers = "

".join([f"【{r['provider']}からの情報】
{r['answer']}" for r in valid_results])
...
"""
        ...
        try:
            synthesis_result = self.connector.generate_code(prompt=synthesis_prompt, max_tokens=1500)
            answer = synthesis_result["code"] if synthesis_result["success"] else valid_results[0]["answer"]
        except:
...
"""
    ...
    def _calculate_confidence(self, rounds: List[Dict[str, Any]]) -> float:
        ...
        if not rounds:
            return 0.0
        confidences = [r["synthesis"]["confidence"] for r in rounds if "synthesis" in r]
        if not confidences:
            return 0.0
