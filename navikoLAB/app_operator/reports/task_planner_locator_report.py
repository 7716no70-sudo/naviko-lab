...
def main() -> None:
    ...
    for path in (ROOT / "navikoLAB").rglob("*.py"):
        ...
        name_hit = "planner" in path.name.lower() or "task" in path.name.lower()
    ...
    report = {
        "status": "completed",
        "phase": "Phase11-4A TaskPlanner Locator Report",
        ...
        "file_delete": False,
        "next_phase": "Phase11-4B Existing TaskPlanner Minimal Patch Connection",
    }

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_path = REPORT_DIR / f"task_planner_locator_report_{timestamp}.json"
    ...
    print("=== TaskPlanner Locator Report ===")
