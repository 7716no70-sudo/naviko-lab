...
from navikoLAB.app_operator.app_operator_workspace_save_adapter import (
    run_workspace_save_adapter_diagnostics,
)
...
def main():
    ...
    report["report_phase"] = "Phase8-6 Workspace Save Adapter Report"

    out = REPORT_DIR / f"app_operator_workspace_save_adapter_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    out.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    print("=== AppOperator Workspace Save Adapter Report ===")
